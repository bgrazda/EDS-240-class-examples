...
jobs_clean |> 
  ...
  scale_y_continuous(labels = label_currency(accuracy = 1, scale = 0.001, suffix = "k"), limits = c(0, 250000)) +
  geom_text(aes(label = dollar(total_earnings)),
            hjust = -.2) +
  coord_flip()
```
...
jobs_clean |>
  ...
  mutate(sex = str_remove(string = group, pattern = "total_earnings_")) |> 
  ggplot(aes(x = occupation, y = earnings_by_group, colour = sex)) +
  geom_point(position = position_dodge(width = .8)) +
  geom_linerange(aes(ymin = 0, ymax = earnings_by_group), position = position_dodge(width = .8, preserve = 'total')) +
  coord_flip()
```
